...
function mapValue(value, oldMin, oldMax, newMin, newMax) {
    ...
    // Round the mapped value to the nearest hundredth and convert it back to a number
...
function createNoteFromPlane(altitude) {
    const noteIndex = mapValue(altitude, 1000, 35000, 0, 22);
    console.log("note index", Math.round(noteIndex), NOTES[Math.round(noteIndex)])
    return NOTES[Math.round(noteIndex)]
}
...
async function getPlanesAroundMe(lat, lon, fence=DISTANCE) {
    url = `${PLANES_BASE_URL}?lat=${lat}&lon=${lon}`;
    const resp = await fetch(url, {headers: {accept: "application/json"}});
    const planeData = await resp.json();
    return planeData;
}
...
function haversineDistance(lat1, lon1, lat2, lon2) {
    ...
    const earthRadius = 6371; // Mean radius of Earth (km)
